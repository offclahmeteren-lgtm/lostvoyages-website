"""
Lost Voyages — Zanzibar PDF Broşürü
Güney Kore broşürüyle aynı stil: krem zemin, Montserrat + ReklameScript, hero fotoğraf.
Tarihsiz versiyon.
"""
...
W, H = A4
OUTPUT   = "/Users/ahmeterenvci/lostvoyages-website/zanzibar-brosur.pdf"
HERO_IMG = "/Users/ahmeterenvci/lostvoyages-website/images/gallery/zb-2.jpg"
...
_, gk_h = reklame(cv, "Zanzibar", W/2, IY + 7.4*cm, 54, rgba=(255,255,255,255), align="center")

T(cv,"M N E M B A   ·   S T O N E   T O W N   ·   J O Z A N I   ·   P A J E",
  W/2, IY+4.6*cm,"M-Light",10.5,WHITE,"center")

T(cv,"6 Gece / 7 Gün   ·   Uçak Hariç   ·   Maks. 20 Kişi",
...
# ── Bilgi alanı ───────────────────────────────────────────────────────────────
STRIP_H   = 2.2*cm
STRIP_TOP = 1*cm + STRIP_H + 3
INFO_TOP  = IY - 5*mm
INFO_BOT  = STRIP_TOP + 0.25*cm
...
_, prh = reklame(cv, "1.645 €", LCX, cy, 38, rgba=(26,23,48,255), align="center")
cy -= prh + 4
...
T(cv,"kalan gezi öncesinde  ·  tek kişilik oda farkı +400 €",
  LM+0.2*cm, cy,"M-Light",7.5,MUTED, max_w=LCW-0.3*cm)
cy -= 10
cy_left_end = cy

badges = [
    ("Rota",    "Mnemba · Stone Town · Jozani · Nakupenda · Paje"),
    ("Süre",    "6 Gece / 7 Gün"),
    ("Rehber",  "Yerel & Türk Tur Lideri"),
    ("Konak.",  "Butik Oteller (Kahvaltı Dahil)"),
    ("Uçak",    "Hariç — ~900–1.000 € bütçe önerilir"),
]
by = INFO_TOP - 0.2*cm
...
g_imgs = [f"{IMGS}/gallery/zb-{i}.jpg" for i in [1,3,4,5,6]]
g_imgs.append(f"{IMGS}/gallery/zb-2.jpg")
gw = W / len(g_imgs)
...
DAYS = [
    ("1","✈","Zanzibar'a Varış",[
        ("Varış",  "Havalimanı Karşılama & Transfer", "Tur liderimiz ve yerel rehberimiz havalimanında sizi karşılıyor"),
        ("Akşam",  "Otel Check-in & Serbest Zaman",  "İlk Zanzibar akşamını sahilde ya da otelde dinlenerek geçiriyoruz"),
    ]),
    ("2","🐬","Mnemba · Baharat · Stone Town",[
        ("Sabah",     "Mnemba Adası — Yunus & Şnorkel",  "Özel tekneyle açılıyoruz; yunus sürüleriyle ve kristal Hint Okyanusu'nda şnorkel"),
        ("Öğle Son.", "Baharat Bahçesi",                  "Karanfil, tarçın, vanilya — Zanzibar'ı 'Baharat Adası' yapan aromalar"),
        ("Akşam",     "Stone Town & Baharat Müzesi",      "UNESCO Dünya Mirası; dar sokaklar, tarihi kapılar ve baharat ticareti müzesi"),
    ]),
    ("3","🦁","Mikumi Safari (Opsiyonel +450 €)",[
        ("Sabah",     "Zanzibar → Dar es Salaam Uçuşu",  "Küçük uçakla Tanzanya anakarasına geçiş"),
        ("Gün Boy.", "Mikumi Milli Parkı — Jeep Safari",  "Aslan, fil, zürafa, zebra, su aygırı — Tanzanya'nın en ulaşılabilir safarisi"),
        ("Akşam",    "Zanzibar'a Dönüş",                  "Dar es Salaam üzerinden adaya geri dönüyoruz"),
    ]),
    ("4","🐢","Köyler · Kaplumbağalar · Mtende",[
        ("Sabah",     "Mkunguni & Kanga Balıkçı Köyleri", "Geleneksel ahşap tekneler, taze av ve gerçek Zanzibar yaşamı"),
        ("Öğle Son.", "Deniz Kaplumbağalarıyla Yüzme",    "Sığ berrak sularda dev deniz kaplumbağalarıyla yüzme"),
        ("Akşam",     "Mtende Plajı",                     "Turkuaz sular, bembeyaz kum — Hint Okyanusu kıyısında serbest zaman"),
    ]),
    ("5","🏝️","Prison Island · Nakupenda Sandbank",[
        ("Sabah",     "Prison Island — Aldabra Kaplumbağaları", "100+ yıllık dev Aldabra kaplumbağalarıyla buluşuyoruz"),
        ("Öğle Son.", "Nakupenda Sandbank'a Yolculuk",          "Swahili'de 'Seni Seviyorum' — okyanusun ortasındaki kumsal adacık"),
        ("Akşam",     "Sandbank'ta Şnorkel & Serbest Zaman",    "Mercan resifleri, tropikal balıklar ve bitmek bilmeyen turkuaz"),
    ]),
    ("6","🐒","Jozani · Mangrove Kano · Paje",[
        ("Sabah",     "Jozani Ormanı & Kolobus Maymunları",    "Yalnızca Zanzibar'da yaşayan nadir Kırmızı Kolobus Maymunları"),
        ("Öğle Son.", "Mangrove Kanallarında Kano",            "Sessiz kanallar, egzotik kuşlar — mangrove ormanının derinliklerine"),
        ("Akşam",     "Paje Sahilinde Son Akşam",              "Kitesörf rüzgarı, okyanus günbatımı — Zanzibar'a veda"),
    ]),
    ("7","✈","Ayrılış",[
        ("Sabah",    "Kahvaltı & Otel Check-out", "Son Zanzibar kahvaltısı — bavulları toplarken adanın kokusunu içimize çekiyoruz"),
        ("Transfer", "Havalimanı Transferi",       "Zanzibar sizi bırakmaz ama siz yola çıkarsınız — bu kez bambaşka birisiniz"),
    ]),
]

COL_W = (CW - 0.6*cm) / 2
COL_X = [LM, LM + COL_W + 0.6*cm]
# Sol: 1,2,3,4  Sağ: 5,6,7
...
if BOX_H >= 1.8*cm:
    R(cv, bx, bby, CW, BOX_H, fill=HexColor("#F0F8FF"), stroke=GOLD, r=5, sw=1)
    R(cv, bx, bby, 5, BOX_H, fill=GOLD)
    highlights = [
        ("Yunus Gözlemi",    "Mnemba'da açık denizde"),
        ("Prison Island",    "Dev Aldabra kaplumbağaları"),
        ("Nakupenda",        "Okyanus ortasında sandbank"),
    ]
    TITLE_H = 0.75*cm
    SPOT_H  = 0.65*cm
    CONTENT_H = TITLE_H + 0.2*cm + SPOT_H
    content_top = bby + BOX_H/2 + CONTENT_H/2
    reklame(cv, "Öne Çıkan Deneyimler", bx+CW/2, content_top, 13,
...
INC = [
    "6 gece kahvaltı dahil konaklama (butik oteller)",
    "Tüm transferler & ulaşım",
    "Tur lideri + yerel rehber (tüm program)",
    "Tüm giriş ücretleri (Jozani, müzeler, bahçeler)",
    "Mnemba Adası — yunus gözlem & şnorkel (tekne)",
    "Prison Island — Aldabra kaplumbağaları",
    "Stone Town rehberli tur & Baharat Müzesi",
    "Jozani Ormanı & Kolobus Maymunları",
    "Kaplumbağa yüzme aktivitesi",
    "Nakupenda Sandbank şnorkel turu",
    "Mangrove kano turu",
    "Tüm bahşişler",
]
EXC = [
    "Uçak biletleri (~900–1.000 € önerilir)",
    "Kapıda vize — 50 USD (yeşil pasaport: ücretsiz)",
    "Mikumi Safari (opsiyonel, +450 €)",
    "Öğle & akşam yemekleri",
    "Seyahat sigortası (zorunlu)",
]
...
if remaining_h > 0.5*cm:
    ...
    T(cv, "Akşam yemeklerini rehberimizle birlikte keşfedebilirsiniz — Stone Town'ın taze deniz mahsulleri sizi bekliyor!",
...
T(cv, "ZANZİBAR", LM+0.35*cm, cy-0.42*cm,"M-Bold",8.5,NAVY)
T(cv, "6 gece butik otel · kahvaltı dahil · otel isimleri katılım sonrası paylaşılır",
  LM+2.8*cm, cy-0.42*cm,"M-Regular",8,MUTED, max_w=CW-3.0*cm)
cy -= 0.75*cm
cy -= 0.5*cm
...
reklame(cv,"1.645 €", LM+0.5*cm, cy-0.12*cm, 26, rgba=(232,173,24,255), align="left")
T(cv,"kişi başı · uçak hariç", LM+0.5*cm, cy-BOX_H2+0.28*cm,"M-Light",8,HexColor("#a09080"))
T(cv,"Ön ödeme: 595 €  ·  kalan gezi öncesinde",
...
for note in [
    "Gün 3 Mikumi Safarisi isteğe bağlıdır; katılmak istemeyenler o gün serbest zaman geçirir.",
    "Tur programı gelgit saatleri ve hava koşullarına göre değişebilir; sıralama korunarak alternatif uygulanabilir.",
...
cards = [
    ...
    ("Özenle Seçilmiş Program",   "Her aktivite bizzat test edilmiş;\nen güzel Zanzibar deneyimi"),
